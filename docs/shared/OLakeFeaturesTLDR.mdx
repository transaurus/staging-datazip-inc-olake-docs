...
1. **Open data formats** – OLake writes raw Parquet and fully ACID snapshots in [Apache Iceberg](/docs/writers/iceberg/catalog/rest) so your lakehouse stays engine-agnostic.
2. **Iceberg writer** – The dedicated Iceberg writer produces exactly-once, rollback-ready Iceberg v2 tables.
...
12. **Developer playground** – A one-click [OLake + Iceberg sandbox](/docs/getting-started/playground) lets you experiment locally with Trino, Spark, Flink or Snowflake readers.


### 2. Source-level features

1. **PostgreSQL** – Full loads and `pgoutput-based` CDC for RDS, Aurora, Supabase, etc. See the [Postgres connector](/docs/connectors/postgres).
2. **MySQL** – Full loads plus `binlog` CDC for MySQL RDS, Aurora and older community versions. See the [MySQL connector](/docs/connectors/mysql).
3. **MongoDB** – High-throughput `oplog` capture for sharded or replica-set clusters. See the [MongoDB connector](/docs/connectors/mongodb).
4. **Optimised chunking strategies** –
    - **MongoDB** – `Split-Vector`, `Bucket-Auto` & `Timestamp`; details in the blog [What Makes OLake Fast](https://olake.io/blog/what-makes-olake-fast).
...
6. We map each of database datatypes to respective Iceberg datatypes so your source schema remains more or less unaffected by the type conversion from a source database to Iceberg. See for:
    - [Postgres type mapping](/docs/connectors/postgres#data-type-mapping)
    - [MySQL type mapping](/docs/connectors/mysql#data-type-mapping)
    - [MongoDB type mapping](/docs/connectors/mongodb#data-type-mapping)

### 3. Destination-level features

1. **Apache Iceberg** – Primary target format, can write to AWS S3, Azure, GCS: see [Iceberg Writer docs](/docs/writers/iceberg/catalog/rest).
   
2. **Catalog options:**
    - **AWS Glue** – [Glue catalog guide](/docs/writers/iceberg/catalog/glue)
    - **REST catalog** – [REST guide](/docs/writers/iceberg/catalog/rest)
    ...
    - **S3 Table Bucket (`SigV4`)** – [S3 Tables Guide](/docs/writers/iceberg/catalog/rest?rest-catalog=s3-tables)
    - **Unity Catalog (Databricks)** – [Unity Catalog Guide](/docs/writers/iceberg/catalog/rest?rest-catalog=unity)

3. **Plain Parquet** – Write partitioned Parquet to S3 or Google Cloud Storage (GCS) with the [Parquet Writer](/docs/writers/parquet/config).
4. **Query engines** – Any Iceberg v2-aware tool (Trino, Spark, Flink, Snowflake, etc.) can query OLake outputs immediately. See [here](/docs/getting-started/quickstart) for more
...
4. **Roadmap tracker** – See the live [OLake roadmap](https://github.com/datazip-inc/olake/?tab=readme-ov-file#-roadmap-highlights).
