---
title: "Parquet Data Partitioning for S3 | OLake Documentation"
description: Learn how to configure flexible Parquet data partitioning in OLake using regex patterns to optimize S3 folder structure and query performance.
...
OLake supports data partitioning when writing to S3. You can define a `partition_regex` during `schema changes` in OLake UI or in `streams.json` in OLake CLI. This regex determines how data is partitioned into folders within your S3 bucket. Partitioning is defined on a per-stream (table or collection) basis.

<HighPartitionWarning/>

<Tabs queryString="s3-partition">

<TabItem value="OLake-UI" label="Partition in OLake UI" default>

### Sample `Partitioning configuration` in OLake UI
![Partitioning regex for dummy_users stream: year, month, day, age field data lake structure](/img/docs/s3/s3-partition-config.webp)


</TabItem>
<TabItem value="OLake-CLI" label="Partition in OLake CLI" default>
...
```json
{
  "selected_streams": {
    "namespace": [
      {
        "stream_name": "dummy_users",
        "partition_regex": "/year={now(),,YYYY}/month={now(),,MM}/day={now(),,DD}/age={age,,}",
        "normalization": true
      }
    ]
  },
...
*Result of the above partition:*

<p style={{ border: "1px solid #ccc", borderRadius: "3px", display: "inline-block" }}>
  <img src="/img/docs/s3/s3-partition-gcs.webp" alt="olake-s3-partition" loading="lazy" decoding="async" />
</p>
...
For the latest updates or to add support for additional timestamp formats, please refer to the [corresponding implementation](https://github.com/datazip-inc/olake/blob/master/utils/typeutils/reformat.go) or [create an issue](https://github.com/datazip-inc/olake/issues).
