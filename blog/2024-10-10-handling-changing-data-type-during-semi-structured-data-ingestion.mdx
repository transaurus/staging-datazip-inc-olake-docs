...
title: "Handling Changing Data Types in Semi-Structured Data Ingestion"
description: "Explore 7 techniques to manage polymorphic keys and evolving data types in semi-structured data ingestion. Learn schema enforcement, type promotion, and more."
image: /img/blog/cover/handling-changing-data-type-during-semi-structured-data-ingestion-cover.webp
authors: [priyansh]
...
![Illustration showing irregular blocks being transformed into an organized, evenly spaced grid, representing data normalization or transformation](/img/blog/cover/handling-changing-data-type-during-semi-structured-data-ingestion-cover.webp)
...
| event_time_long | Long (milliseconds since epoch) |
...
| id | name | age | purchase_amount | city | pincode |
| --- | --- | --- | --- | --- | --- |
...
| id | name | age | purchase_amount_1 | purchase_amount_2 | purchase_amount_3 | city | pincode |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
| **id** | **name** | **age** | **purchase_amount** | **address** |
| --- | --- | --- | --- | --- |
...
![Data flow diagram showing strict type checking with a message broker and dead letter queue; handling rejected data with either user intervention or automated schema migration](/img/blog/2024/10/handling-changing-data-type-1.webp)
...
| **_id** | **name** | **attribute_versions** | **attribute_size** |
| --- | --- | --- | --- |
...
| **_id** | **attribute_size_array** | **attribute_size_dimension** | **attribute_size_value** |
| --- | --- | --- | --- |
| 11223 | 5 | NULL | NULL |
| 11223 | 10 | NULL | NULL |
| 11223 | 15 | NULL | NULL |
...
* **Schema Enforcement (When Enabled)**:
    ...
    * ![Table excerpt stating that on Destinations V2, incompatible data types in new or updated rows cause row errors and errors in the airbyte_meta field](/img/blog/2024/10/handling-changing-data-type-2.webp)
...
![Dashboard showing multiple data pipeline connections (MongoDB to Clickhouse, S3, Local CSV, MySQL) with health, last sync, and enable/disable status](/img/blog/2024/10/handling-changing-data-type-3.webp)
...
![Stream selection UI displaying an enabled 'testdatapriyansh' stream set to incremental, append-plus-dedupe sync, with cursor and primary key fields specified](/img/blog/2024/10/handling-changing-data-type-4.webp)

And the table after first full sync:

![Database table grid for 'testdatapriyansh' displaying rows with columns '_id', 'name', and 'attributes' containing JSON objects with heterogeneous structures](/img/blog/2024/10/handling-changing-data-type-5.webp)
...
![Database grid for 'testdatapriyansh' showing columns '_id', 'name', and 'attributes,' with several rows containing JSON values and two rows having NULL attributes](/img/blog/2024/10/handling-changing-data-type-6.webp)
...
![Schema update modal showing the 'attributes' field in stream 'testdatapriyansh' (namespace 'priyansh') changing data type from object to string](/img/blog/2024/10/handling-changing-data-type-7.webp)
...
![Database grid for 'testdatapriyansh' where the 'attributes' column contains mixed data types JSON objects, integers, and strings showing schema inconsistency](/img/blog/2024/10/handling-changing-data-type-8.webp)
...
![Pipeline error log for MongoDB to MySQL, showing a DECIMAL data truncation error due to trying to insert non-numeric JSON content into a numeric column](/img/blog/2024/10/handling-changing-data-type-9.webp)
...
![Error screen for 'priyansh.testdatapriyansh' showing a varchar-to-int type mapping error and suggesting a transformation or destination column type update](/img/blog/2024/10/handling-changing-data-type-10.webp)

Hevo showing error when the `INT` column started to receive string data OR an array of objects with auto-mapping enabled.

![Data mapping UI warning: source 'varchar(62)' field 'xyz' is mapped to incompatible destination type 'int', with suggestions to resolve the data type mismatch](/img/blog/2024/10/handling-changing-data-type-11.webp)
...
![Database query result for 'testdatapriyansh' displays _id and xyz as varchar(191) types, with rows storing both numeric ('123') and non-numeric ('two') string values in the xyz field](/img/blog/2024/10/handling-changing-data-type-12.webp)
...
![MongoDB shell output of .find() on testdatapriyansh, displaying documents where the xyz field contains both a string ('two') and a number (123)](/img/blog/2024/10/handling-changing-data-type-13.webp)
...
![Example demonstrating JSON to table conversion: JSON fields (including an array and a nested object) are mapped to columns, with arrays and objects stored as JSON strings in their respective table cells](/img/blog/2024/10/handling-changing-data-type-14.webp)
