...
title: "How to Query Semi-Structured JSON Data in Snowflake | OLake Guide"
description: "Learn to query JSON in Snowflake using VARIANT, FLATTEN, LATERAL FLATTEN, and JSON functions. Includes loading methods and best practices for nested data."
image: /img/blog/cover/querying-json-in-snowflake-cover.webp
authors: [priyansh]
...
![JSON data being imported into Snowflake](/img/blog/cover/querying-json-in-snowflake-cover.webp)
...
![SQL query returning rows with embedded JSON objects in a table](/img/blog/2024/09/querying-json-in-snowflake-1.webp)
...
![Query output with extracted JSON name and age fields as columns](/img/blog/2024/09/querying-json-in-snowflake-2.webp)
...
![Snowflake SQL query using JSON_ARRAY_ELEMENTS for array processing](/img/blog/2024/09/querying-json-in-snowflake-4.webp)
...
![Snowflake query result showing array element extraction and flattening](/img/blog/2024/09/querying-json-in-snowflake-5.webp)
...
| **Feature** | **PARSE_JSON** | **TRY_PARSE_JSON** |
| --- | --- | --- |
...
![Snowflake SQL command creating table with a VARIANT column for JSON data](/img/blog/2024/09/querying-json-in-snowflake-6.webp)
...
![Snowflake SQL command inserting a parsed nested JSON object into a VARIANT column, with customer and order details](/img/blog/2024/09/querying-json-in-snowflake-7.webp)
...
![Table showing customer name, first order date, and first item name extracted from nested JSON](/img/blog/2024/09/querying-json-in-snowflake-8.webp)
...
![Table displaying address fields (street, city, state, postal code) in separate columns](/img/blog/2024/09/querying-json-in-snowflake-9.webp)
...
![Table with columns for customer name, order ID, and first item name, showing one row of data](/img/blog/2024/09/querying-json-in-snowflake-10.webp)
...
![Table showing customer name, order ID, and first item name for a single order](/img/blog/2024/09/querying-json-in-snowflake-11.webp)
...
![Table listing customer name, item ID, item name, and item price, with two rows of product purchase data](/img/blog/2024/09/querying-json-in-snowflake-12.webp)
...
![Table listing customer name, item name, and item category across multiple rows for each category an item belongs to](/img/blog/2024/09/querying-json-in-snowflake-13.webp)
...
![Table displaying customer name and the first item's primary category, showing one row: John Doe, electronics](/img/blog/2024/09/querying-json-in-snowflake-14.webp)
...
![Table displaying customer name, order ID, and total order price for each order](/img/blog/2024/09/querying-json-in-snowflake-15.webp)
...
![Table showing customer name, order ID, and the number of items per order, with both orders containing 2 items](/img/blog/2024/09/querying-json-in-snowflake-16.webp)
...
![Table showing customer name, order ID, and average item price for each order](/img/blog/2024/09/querying-json-in-snowflake-17.webp)
...
![Table displaying columns for variant JSON data, variant number, and variant date, with one row holding JSON containing name Alice, number 123, and date 2024-09-23](/img/blog/2024/09/querying-json-in-snowflake-18.webp)
...
![Table displaying one column named PARSED_ARRAY, with a single row containing the array: electronics, computers, accessories](/img/blog/2024/09/querying-json-in-snowflake-19.webp)
...
![Table showing a single column called CATEGORY, with rows for electronics, computers, and accessories](/img/blog/2024/09/querying-json-in-snowflake-20.webp)
...
![Table of customer name with corresponding item names array: Laptop, Mouse, Keyboard, and Monitor](/img/blog/2024/09/querying-json-in-snowflake-21.webp)
...
![Table showing customer name, item name (Laptop, Mouse, Keyboard, Monitor), and a category count of 2 for each item](/img/blog/2024/09/querying-json-in-snowflake-22.webp)
...
![Query result indicating the view V_FLATTENED_ORDERS was successfully created](/img/blog/2024/09/querying-json-in-snowflake-23.webp)
...
WHERE 
    table_schema = 'PUBLIC'
    AND table_catalog = 'OLAKE_DB';
```

![SQL query and result showing the view V_FLATTENED_ORDERS in the PUBLIC schema of the PRIYANSH_DB database](/img/blog/2024/09/querying-json-in-snowflake-24.webp)

**Note**: `OLAKE_DB` is the name of the database, replace it with your own.
...
![Data table showing four orders by John Doe, listing order ID, item, price, and categories for each purchased item](/img/blog/2024/09/querying-json-in-snowflake-25.webp)
...
| **customer_name** | **order_id** | **item_name** | **item_price** | **rewards_points** |
| --- | --- | --- | --- | --- |
...
Refer here for more - [https://docs.snowflake.com/en/sql-reference/functions/array_contains](https://docs.snowflake.com/en/sql-reference/functions/array_contains)

**Expected Output**:

![Database query results table showing five John Doe orders with item name, price, and single category for each row](/img/blog/2024/09/querying-json-in-snowflake-26.webp)
...
| **customer_name** | **order_id** | **item_name** | **tag** |
| --- | --- | --- | --- |
...
| **customer_name** | **total_spent** | **rewards_points** |
| --- | --- | --- |
...
![Database query results table showing two orders by John Doe, each with an order ID, an array of item prices, and total order price.](/img/blog/2024/09/querying-json-in-snowflake-27.webp)
...
![Snowflake SQL command creating table with a VARIANT column for JSON data](/img/blog/2024/09/querying-json-in-snowflake-29.webp)
...
![Database query results table with one row: customer ID "C123" and order IDs listed as an array: "O1001", "O1002", "O1003".](/img/blog/2024/09/querying-json-in-snowflake-30.webp)
...
![Database query results table with one row for customer ID "C123", first order "O1001", and total orders as 3.](/img/blog/2024/09/querying-json-in-snowflake-31.webp)
...
![Table displaying address fields (street, city, state, postal code) in separate columns](/img/blog/2024/09/querying-json-in-snowflake-32.webp)
...
![Table with columns for extracted name, item, and price from JSON data](/img/blog/2024/09/querying-json-in-snowflake-33.webp)
