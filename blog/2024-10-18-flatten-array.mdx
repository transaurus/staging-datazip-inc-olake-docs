---
slug: flatten-array
title: "How to Flatten Object Types & Query Arrays in Semi-Structured Data"
description: "Discover multiple methods to flatten nested JSON and query arrays for effective data extraction using Python, PySpark, pandas, and popular ETL tools."
...
![Arrow illustrating conversion from nested JSON data to a structured table format](/img/blog/cover/flatten-array-cover.webp)
...
![Nested JSON data converted to a table where each column represents a level 1 key from the original JSON](/img/blog/2024/11/flatten-array-1.webp)
...
![Nested JSON data is transformed so each top-level key maps to a column in a flat table](/img/blog/2024/11/flatten-array-2.webp)

**The schema:**

![Database table schema with standard fields plus a JSON column for project data](/img/blog/2024/11/flatten-array-3.webp)
...
![Nested JSON data is transformed so all keys and arrays are flattened into columns of a single denormalized table.](/img/blog/2024/11/flatten-array-4.webp)
...
![Denormalized table schema with flattened columns for each project and task, using hierarchical column names from nested JSON](/img/blog/2024/11/flatten-array-5.webp)
...
![Nested JSON data is decomposed into multiple normalized tables joined by primary and foreign keys](/img/blog/2024/11/flatten-array-6.webp)
...
![ER diagram with normalized tables: main, projects, and project tasks, using keys and foreign keys to represent hierarchical relationships](/img/blog/2024/11/flatten-array-7.webp)
...
![Diagram with a panda holding a 'JSON' sign, illustrating nested JSON data being fully flattened into columns of a single main table](/img/blog/2024/11/flatten-array-8.webp)
...
![Terminal output of a pandas DataFrame showing flattened task and project data with columns for task, project, and metadata fields](/img/blog/2024/11/flatten-array-9.webp)
...
![Diagram showing PySpark used to flatten nested JSON data into columns of a single main table](/img/blog/2024/11/flatten-array-10.webp)
...
![PySpark DataFrame printed in tabular format, with columns for user, project, and task details after flattening nested JSON](/img/blog/2024/11/flatten-array-11.webp)
...
![Diagram showing PySpark flattening JSON by extracting only level 1 keys into individual columns of a main table](/img/blog/2024/11/flatten-array-12.webp)
...
![PySpark table output with columns for id, name, contact, location, and projects, where nested fields remain grouped within the main column](/img/blog/2024/11/flatten-array-13.webp)
...
![PySpark DataFrame output with flattened columns for user, contact, location, project, and project task fields, showing one record per row](/img/blog/2024/11/flatten-array-14.webp)
...
![Schema setup screen for a data stream, showing checkboxes for each field, field types, and complex JSON fields like objects and arrays](/img/blog/2024/11/flatten-array-15.webp)

**Destination Schema (MySQL):**

![Database table structure showing columns with data types including json, text, decimal, varchar, and timestamp](/img/blog/2024/11/flatten-array-16.webp)
...
![Data pipeline UI workspace with a Flatten JSON transformation block selected, allowing configuration for parsing JSON into flat database columns](/img/blog/2024/11/flatten-array-17.webp)
...
![Data pipeline UI showing a Flatten JSON transformation with filters for event type and field name, focused on flattening the 'projects' array field](/img/blog/2024/11/flatten-array-18.webp)
...
![Side-by-side view of source and transformed JSON, showing a sample event's nested fields and types before data pipeline deployment](/img/blog/2024/11/flatten-array-19.webp)

Our Data as stored in mongoDB.

![MongoDB pretty print output showing a document with nested user, location, and projects arrays containing further nested tasks](/img/blog/2024/11/flatten-array-20.webp)
...
![ETL pipeline setup showing JSON replication options either as columns or as strings and a warning that this cannot be changed later](/img/blog/2024/11/flatten-array-21.webp)

So we went ahead with option 2 this time as we had some array fields,

![MongoDB shell output showing pretty-formatted documents with nested fields including contact info, location with coordinates, and projects with nested tasks](/img/blog/2024/11/flatten-array-22.webp)
...
![UI with four cards offering Quickstart no-code models, dbt Core, dbt Cloud, and Coalesce integrations for data transformation, plus a MySQL warning](/img/blog/2024/11/flatten-array-23.webp)
