...
image: /img/blog/cover/postgres-to-iceberg-using-glue-cover.webp
authors: [rohan]
tags: [iceberg, postgres, glue, olake]
---

![Postgres to Apache Iceberg with AWS Glue integration](/img/blog/cover/postgres-to-iceberg-using-glue-cover.webp)
...
- PostgreSQL 10 or higher (pgoutput is native from v10+)
...
#### 🎯 Create a Publication and a Logical Replication Slot

OLake reads changes via a logical replication slot (pgoutput) and requires a publication to define which tables to replicate:
```sql
...
```
This starts tracking changes from the current WAL position. Ensure the publication name matches your OLake `source.json`.
...
![Amazon S3 console showing contents of test-olake-pg bucket, including olake_usecase_test.db and Unsaved folders](/img/blog/2025/04/postgres-to-iceberg-using-glue-1.webp)
...
![Amazon S3 console displaying olake_test_table directory containing 'data/' and 'metadata/' folders within an S3 bucket](/img/blog/2025/04/postgres-to-iceberg-using-glue-2.webp)
...
![AWS Athena Query Editor running SQL queries on 'olake_usecase_test' database and 'olake_test_table', showing query results with row count and query status](/img/blog/2025/04/postgres-to-iceberg-using-glue-3.webp)
